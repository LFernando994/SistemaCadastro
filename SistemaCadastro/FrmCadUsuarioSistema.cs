...
{
    public partial class FrmCadUsuarioSistema : Form
    {
        static int pinDeAcesso = 1010;
        AcessosUsuario daoAcessos = new AcessosUsuario();
        UsuariosSistema secretariaxml = new UsuariosSistema();
        UsuariosSistema novaSecretaria;
        AcessosUsuario acessos;
        int id;
        bool edit; //Olha se é edição ou cadastro de uma secretaria
        UsuariosSistema secretariaEdit;
   

        public FrmCadUsuarioSistema() // Construtor para cadastro
        {
            try
            {
                InitializeComponent();
